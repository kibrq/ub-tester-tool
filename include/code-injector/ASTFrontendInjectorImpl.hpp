...
inline std::string
getArgAsString(const std::string& Arg, const clang::ASTContext*) {
  return Arg;
}

inline std::string getArgAsString(
    const clang::SourceRange& Range, const clang::ASTContext* Context) {
  return getRangeAsString(Range, Context);
}

inline std::string
getArgAsString(const clang::Expr* Ex, const clang::ASTContext* Context) {
...
template <typename T>
inline void generateArgumentsForSubstitution_(
    const clang::ASTContext* Context, SubArgs& Vec, const T& Arg) {
  Vec.push_back(getArgAsString(Arg, Context));
}

template <typename T, typename U>
inline void generateArgumentsForSubstitution_(
    const clang::ASTContext* Context, SubArgs& Vec, const T& Arg1,
    const U& Arg2) {
...
template <typename T>
inline SubArgs generateArgumentsForSubstitution(
    const clang::ASTContext* Context, const T& Arg) {
  SubArgs Result;
...
template <typename T, typename U, typename... W>
inline SubArgs generateArgumentsForSubstitution(
    const clang::ASTContext* Context, const T& Arg1, const U& Arg2, W... ws) {
  SubArgs Result;
...
void ASTFrontendInjector::substitute(
    ...
    Args... as) {
  substitute(
      Context, BeginLoc, SourceFormat, OutputFormat,
